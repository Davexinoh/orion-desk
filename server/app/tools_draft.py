...
def _complete(kind: str, intent: str, prior: list[str]) -> str | None:
    ...
    system = (
        ...
        "If kind is list: a waiting list from the mail lines only. "
        "Do not invent names, companies, or emails. "
        "If a line is missing, omit it. "
        "Do not write a letter. Do not use Hi / Best / Subject. "
        "If they asked for a recipe: dish, servings, timing, ingredients, steps. "
        "Do not title it Orion Desk Documentation. "
        "Never say the work was sent. Plain text."
    )
    user = (
        f"Intent:\n{intent}\n\n"
