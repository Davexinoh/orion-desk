...
def _web_search_live(intent: str, key: str) -> dict[str, Any]:
    query = " ".join((intent or "").split())[:120] or "company research"
    try:
    ...
    n = len(titles)
    evidence = f"Tavily: {n} sources on {topic} — " + "; ".join(titles)
    return {"ok": True, "evidence": _one_line(evidence, 120), "mode": "live"}
